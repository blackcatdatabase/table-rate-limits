...
use BlackCat\Database\Packages\RateLimits\Repository\RateLimitRepositoryInterface;
...
class RateLimitRepository implements RateLimitRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'subject_type', 'subject_id', 'name', 'window_size_sec' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\RateLimits\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'limit_count', 'active' ]);
          return count($rows);
        }
    ...
    public function getBySubjectTypeAndSubjectIdAndNameAndWindowSizeSec(string $subjectType, string $subjectId, string $name, int $windowSizeSec, bool $asDto = false): array|\BlackCat\Database\Packages\RateLimits\Dto\RateLimitDto|null {
        return $this->getByUnique([ 'subject_type' => $subjectType, 'subject_id' => $subjectId, 'name' => $name, 'window_size_sec' => $windowSizeSec ], $asDto);
    }
    ...
    public function getIdBySubjectTypeAndSubjectIdAndNameAndWindowSizeSec(string $subjectType, string $subjectId, string $name, int $windowSizeSec) {
        $row = $this->getBySubjectTypeAndSubjectIdAndNameAndWindowSizeSec($subjectType, $subjectId, $name, $windowSizeSec, false);
