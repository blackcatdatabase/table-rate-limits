...
namespace BlackCat\Database\Packages\RateLimits\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\RateLimits\Definitions;
use BlackCat\Database\Packages\RateLimits\Criteria;
use BlackCat\Database\Packages\RateLimits\Dto\RateLimitDto as Dto;
use BlackCat\Database\Packages\RateLimits\Mapper\RateLimitDtoMapper as RowMapper;
...
final class RateLimitRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\RateLimits\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'subject_type', 'subject_id', 'name', 'window_size_sec' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\RateLimits\Definitions::table());
            return $bulk->upsertMany($rows, [ 'subject_type', 'subject_id', 'name', 'window_size_sec' ], [ 'limit_count', 'active' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\RateLimits\Dto\RateLimitDto|null */
    public function getBySubjectTypeAndSubjectIdAndNameAndWindowSizeSec(string $subjectType, string $subjectId, string $name, int $windowSizeSec, bool $asDto = false): array|\BlackCat\Database\Packages\RateLimits\Dto\RateLimitDto|null {
        $row = $this->getByUnique([ 'subject_type' => $subjectType, 'subject_id' => $subjectId, 'name' => $name, 'window_size_sec' => $windowSizeSec ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\RateLimits\Mapper\RateLimitDtoMapper::fromRow($row);
    }
    public function existsBySubjectTypeAndSubjectIdAndNameAndWindowSizeSec(string $subjectType, string $subjectId, string $name, int $windowSizeSec): bool {
        $where = 't.' . Ident::q($this->db, 'subject_type') . ' = :uniq_subject_type' . ' AND ' . 't.' . Ident::q($this->db, 'subject_id') . ' = :uniq_subject_id' . ' AND ' . 't.' . Ident::q($this->db, 'name') . ' = :uniq_name' . ' AND ' . 't.' . Ident::q($this->db, 'window_size_sec') . ' = :uniq_window_size_sec';
        return $this->exists($where, [ 'uniq_subject_type' => $subjectType, 'uniq_subject_id' => $subjectId, 'uniq_name' => $name, 'uniq_window_size_sec' => $windowSizeSec ]);
    }
    /** @return int|string|null */
    public function getIdBySubjectTypeAndSubjectIdAndNameAndWindowSizeSec(string $subjectType, string $subjectId, string $name, int $windowSizeSec) {
        $row = $this->getBySubjectTypeAndSubjectIdAndNameAndWindowSizeSec($subjectType, $subjectId, $name, $windowSizeSec);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\RateLimits\Dto\RateLimitDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\RateLimits\Dto\RateLimitDto|null {
