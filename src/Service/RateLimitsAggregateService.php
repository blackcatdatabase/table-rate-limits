...
namespace BlackCat\Database\Packages\RateLimits\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\RateLimits\Dto\RateLimitDto;
use BlackCat\Database\Packages\RateLimits\Mapper\RateLimitDtoMapper;
 ...
 */
final class RateLimitsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private RateLimitRepository $rateLimitRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'rate_limits' => $this->rateLimitRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\RateLimits\ModuleException("Record {$idStr} not found for locking.");
            }
